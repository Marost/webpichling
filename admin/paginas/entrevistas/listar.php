...
$buscar=$_REQUEST["busqueda"];
	...
	if ($_REQUEST["btnbuscar"]=="")
	{
		...
		$rst_query=mysql_query("SELECT * FROM pf_entrevista_mes WHERE id>0 ORDER BY id DESC LIMIT $inicio, $registros;", $conexion);
	...
	{
		$rst_query=mysql_query("SELECT * FROM pf_entrevista_mes WHERE nombre LIKE '%$buscar%' ORDER BY id DESC;", $conexion);
		$num_registros=mysql_num_rows($rst_query);
		...
		$rst_query=mysql_query("SELECT * FROM pf_entrevista_mes WHERE nombre LIKE '%$buscar%' ORDER BY id DESC LIMIT $inicio, $registros;", $conexion);
		$paginas=ceil($num_registros/$registros);
...
<link rel="stylesheet" type="text/css" href="../../css/style-listas.css">
                      ...
                      <td><table width="95%" align="center" cellpadding="5" cellspacing="0" id="cebreado-php">
                        <thead>
                          <tr class="titulo-campo">
                            <th width="80%" height="25">ENTREVISTA</th>
  ...
  ?>
                          <tr<?php echo alt($zebra); $zebra++; ?>>
                            <td width="80%"><p><?php echo stripslashes ($fila["nombre"]); ?></p></td>
