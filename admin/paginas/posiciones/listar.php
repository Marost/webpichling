...
header("Content-Type: text/html; charset=utf-8");
	...
	{
		$rst_query=mysql_query("SELECT * FROM pf_posicion_fija WHERE id>0 ORDER BY posicion ASC;", $conexion);
		...
		$rst_query=mysql_query("SELECT * FROM pf_posicion_fija WHERE id>0 ORDER BY posicion ASC LIMIT $inicio, $registros;", $conexion);
	...
	{
		$rst_query=mysql_query("SELECT * FROM pf_posicion_fija WHERE posicion LIKE '%$buscar%' ORDER BY nombre_completo ASC;", $conexion);
		$num_registros=mysql_num_rows($rst_query);
		...
		$rst_query=mysql_query("SELECT * FROM pf_posicion_fija WHERE posicion LIKE '%$buscar%' ORDER BY nombre_completo ASC LIMIT $inicio, $registros;", $conexion);
		$paginas=ceil($num_registros/$registros);
...
<div id="contenido">
            	<div id="titulo_principal">
            	  <h2>Lista - Posicion</h2>
				</div><!-- FIN TITULO PRINCIPAL -->
                <div id="contenido_total">
                  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
                    <tr>
                      <td width="28%" height="30" align="center"><form id="form1" name="form1" method="get" action="../jugadores - Copia/listar.php">
                        <p>Buscar:
                    ...
                    <tr>
                      <td height="30" align="left"><p><a href="form-agregar.php"><strong>AGREGAR POSICION</strong></a></p></td>
                    </tr>
                    <tr>
                      <td><table width="95%" align="center" cellpadding="5" cellspacing="0" id="cebreado-php">
                        <thead>
                          <tr class="titulo-campo">
                            <th width="80%" height="25">Posiciones</th>
                            <th width="10%" height="25" align="center">ACCIONES</th>
                          </tr>
                        </thead>
                        <tbody>
                          <?php while ($fila=mysql_fetch_array($rst_query)){ ?>
                          <tr<?php echo alt($zebra); $zebra++; ?>>
                            <td width="80%"><p><?php echo stripslashes ($fila["posicion"]); ?></p></td>
                            <td width="10%" align="center">
                            <a href="form-modificar.php?id=<?php echo $fila["id"] ?>" target="mainFrame">
                            <img src="../../images/editar_16.png" width="16" height="16" /></a>
                            &nbsp;
                             <a href="javascript:;" onclick="eliminarRegistro(<?php echo $fila["id"] ?>, '<?php echo stripslashes(htmlspecialchars($fila["posicion"])) ?>');"><img src="../../images/eliminar_16.png" width="16" height="16" /></a>
                            </td>
                          </tr>
                          <?php } ?>
                        </tbody>
