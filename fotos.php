...
$rst_query=mysql_query("SELECT * FROM pf_fotografo WHERE id>0 ORDER BY id ASC", $conexion);
$rst_query1=mysql_query("SELECT * FROM pf_fotografo WHERE id>0 ORDER BY id ASC", $conexion);
?>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
<title>Galería Fotográfica del PICHLING Representaciones</title>
...
p{ font-family:Arial, Helvetica, sans-serif; color:#FF0; font-size:22px;}
h6#12{ color:#FFF; font-size:12px;}
</style>
    </head>
    <body>
	    ...
	    <div class="fp_bgPattern"></div>
		  ...
		  <p>de PICHLING Representaciones</p>
          <p>&nbsp;</p>
          <h6 id="12">Da clic en el nombre para ver las fotos</h6>
          <p>&nbsp;</p>

			<ul id="fp_galleryList" class="fp_galleryList">
            	<?php while($fila_query=mysql_fetch_array($rst_query)){ ?>
                    <li><?php echo $fila_query["nombre_completo"] ?></li>
                <?php } ?>
			</ul>
			<div id="fp_thumbContainer">
				<div id="fp_thumbScroller">
                	<?php while($fila_query1=mysql_fetch_array($rst_query1)){ ?>
                        <div class="container">						
                            	<?php
									$idfotografo=$fila_query1["id"];
                                	$rst_query2=mysql_query("SELECT * FROM pf_fotografo_foto WHERE fotografo=$idfotografo ORDER BY id ASC", $conexion);
								?>
								<?php while($fila_query2=mysql_fetch_array($rst_query2)){ ?>
                                	<div class="content">
                                		<div><a href="#"><img src="imagenes/fotos/escala-150x200.php?imagen=<?php echo $fila_query2["foto"] ?>" alt="imagenes/fotos/<?php echo $fila_query2["foto"] ?>" class="thumb" /></a></div>
                                    </div>
                                <?php } ?>
                        </div>
					<?php } ?>
			...
			<div id="fp_close" class="fp_close">Cerrar</div>
		</div>
